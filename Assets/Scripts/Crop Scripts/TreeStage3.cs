...
{
    public class TreeStage3 : MonoBehaviour
    {
        [SerializeField] private GameObject finalTree; // Prefab for S3 Carrot Stage.
        ...
        private void Update()
        {
            ...
            if (currentTimeOfPlanting > nextPlantStageTime) // If the current time is more then + 5 in hours of when this was first spawn in then it will change to S3 Carrot Stage.
            {
                ...
                Instantiate(finalTree, transform.position, newRotation); // Spawns in dirt prefab with the current transform.position with a rotation of the newRotation variable from above.
